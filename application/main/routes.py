...
def home():
    ...
    if request.method == "POST":
        if search_form.search.data:
            return redirect(
                (url_for("main.search_results", search_term=search_form.search.data))
            )
...
def search_results(search_term):
    ...
    if sq.exact_recipe_match(search_term):
        return redirect(url_for("main.compare_recipes", search_term=search_term))
    # fuzzy search
    ...
    if len(results) > 0:
        ...
        if current_user.is_authenticated:
            ...
            results = results.merge(
                user_results[["user_rating", "recipesID"]], how="left", on="recipesID"
            )
...
def profile():
    ...
    if (
        verify_email_form.submit_verify_email.data
        and verify_email_form.validate_on_submit()
    ):
    ...
    if newsletter_form.submit_newsletter.data and newsletter_form.validate_on_submit():
    ...
    # delete account
    if (
        delete_account_form.submit_delete_account.data
        and delete_account_form.validate_on_submit()
    ):
        if delete_account_form.username.data == current_user.username:
            ...
            flash(
                "You need to confirm account deletion by entering \
                  your username."
            )
            redirect(url_for("main.profile"))
...
def add_or_remove_bookmark(bookmark, origin):
    ...
    if origin == "main.compare_recipes":
        ...
        search_term = session["search_query"]
        return redirect(
            url_for(origin, search_term=search_term, sort_by="Sustainability")
        )
...
def like_recipe(recipe_url):
    ...
    if origin == "main.compare_recipes":
        return redirect(url_for(origin, search_term=search_query, sort_by=sort_by))
    elif origin == "main.search_results":
        return redirect(url_for(origin, search_term=search_query, sort_by=sort_by))
    elif origin == "main.cookbook":
        return redirect(url_for(origin, sort_by=sort_by))
