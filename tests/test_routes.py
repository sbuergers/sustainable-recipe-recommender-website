...
class TestRoutesMain:
    def test_home(self, test_client, par):
        ...
        # Post search terms
        endpoints = ["main.home", "main.search_results", "main.compare_recipes"]
        for (search_term, endpoint) in zip(par.search_terms, endpoints):
            form_data = {"search": search_term}
            r = test_client.post(
                url_for("main.home"), follow_redirects=True, json=form_data
            )
    ...
    def test_search_results(self, test_client, par):
        ...
        for (search_term, endpoint) in zip(par.search_terms, endpoints):
            ...
            if search_term:
                r = test_client.post(
                    url_for("main.search_results", search_term=search_term),
                    follow_redirects=True,
                )
                assert route_meta_tag(r) == endpoint
    ...
    def test_profile(self, test_client, user, pg):
        ...
        assert b"A verification link has been sent to your email address." in r.data
        ...
        for i in range(2):
            old_status = User.query.filter_by(userID=user.userID).first().optin_news
            r = test_client.post(
                url_for("main.profile"),
                data={"submit_newsletter": True},
                follow_redirects=True,
            )
            new_status = User.query.filter_by(userID=user.userID).first().optin_news
        ...
        r = test_client.post(
            url_for("main.profile"),
            data={
                "username": pg.dummy_name + "asdfjkl;",
        ...
        userID = User.query.filter_by(username=pg.dummy_name).first().userID
        assert userID > 0
        form_data = {"username": pg.dummy_name, "submit_delete_account": True}
        r = test_client.post(
            url_for("main.profile"), follow_redirects=True, json=form_data
        )
        user = User.query.filter_by(username=pg.dummy_name).first()
    ...
    def test_add_or_remove_bookmark(self, test_client, pg, user, par):
        ...
        r = test_client.get(
            url_for(
                "main.add_or_remove_bookmark", bookmark=search_term, origin="main.about"
            ),
            follow_redirects=True,
        )
        assert route_meta_tag(r) == "main.home"
    ...
    def test_like_recipe(self, test_client, pg, par):
        ...
        rating = pg.query_user_ratings(user.userID, [par.recipe_tag]).user_rating[0]
        assert rating == 3

        # logged out: Redirect to auth.signin
        logout(test_client)

        origins = ["main.cookbook", "main.search_results", "main.compare_recipes"]
        sort_bys = ["Sustainability"] * 3
        for (origin, sort_by, search_term) in zip(origins, sort_bys, par.search_terms):
            r = test_client.post(
                url_for(
                    "main.like_recipe",
...
class TestRoutesAuth:
    def test_signup(self, test_client, user, pg):
        ...
        r = test_client.post(
            url_for("auth.signup"), data={"username": user.name}, follow_redirects=True
        )
        ...
        r = test_client.post(
            url_for("auth.signup"), data={"email": user.email}, follow_redirects=True
        )
        ...
        # Invalid username
        r = test_client.post(
            url_for("auth.signup"), data={"username": "abc"}, follow_redirects=True
        )
        ...
        r = test_client.post(
            url_for("auth.signup"),
            data={"email": "asdjkflaevzmekfj.com"},
            follow_redirects=True,
        )
        ...
        # Invalid password
        r = test_client.post(
            url_for("auth.signup"), data={"password": "abc"}, follow_redirects=True
        )
        ...
        r = test_client.post(
            url_for("auth.signup"),
            data={"password": user.pw, "confirm_pswd": user.pw + "a"},
            follow_redirects=True,
        )
        ...
        r = test_client.post(
            url_for("auth.signup"), data={"optin_terms": []}, follow_redirects=True
        )
    ...
    def test_terms_and_conditions(self, test_client, user):
        ...
        r = test_client.get(url_for("auth.terms_and_conditions"), follow_redirects=True)
        assert r.status_code == 200
    ...
    def test_reset_password_request(self, test_client, user):
        ...
        login(test_client, user.name, user.pw)
        r = test_client.post(
            url_for("auth.reset_password_request"), follow_redirects=True
        )
        assert r.status_code == 200
        assert route_meta_tag(r) == "main.home"
        ...
        assert r.status_code == 200
        assert route_meta_tag(r) == "auth.signin"
        assert b"Check your email for the instructions to reset your password" in r.data
    ...
    def test_reset_password(self, test_client, user):
        ...
        login(test_client, user.name, user.pw)
        r = test_client.get(
            url_for("auth.reset_password", token="some_token"), follow_redirects=True
        )
        assert r.status_code == 200
        assert route_meta_tag(r) == "main.home"
        ...
        logout(test_client)
        r = test_client.get(
            url_for("auth.reset_password", token="bad_token"), follow_redirects=True
        )
        assert r.status_code == 200
        assert route_meta_tag(r) == "main.home"
        ...
        u = User.query.filter_by(userID=user.userID).first()
        r = test_client.get(
            url_for("auth.reset_password", token=u.get_reset_password_token()),
            follow_redirects=True,
        )
        assert r.status_code == 200
        ...
        r = test_client.post(
            url_for("auth.reset_password", token=u.get_reset_password_token()),
            data={"password": "abc", "confirm_pswd": "abc"},
            follow_redirects=True,
        )
        assert route_meta_tag(r) == "auth.reset_password"
        assert b"Password must be" in r.data
        ...
        r = test_client.post(
            url_for("auth.reset_password", token=u.get_reset_password_token()),
            data={"password": user.pw, "confirm_pswd": user.pw},
            follow_redirects=True,
        )
        assert route_meta_tag(r) == "auth.signin"
    ...
    def test_verify_email(self, test_client, user, pg):
        ...
        if u.confirmed:
            ...
            pg.session.commit()
        r = test_client.post(
            url_for("auth.verify_email", token=u.get_verify_email_token()),
            follow_redirects=True,
        )
        assert route_meta_tag(r) == "main.profile"
        ...
        assert route_meta_tag(r) == "main.home"
        assert b"You need to be logged in in order to verify your email." in r.data
        ...
        if u.confirmed:
            ...
            pg.session.commit()
        r = test_client.post(
            url_for("auth.verify_email", token="bad_token"), follow_redirects=True
        )
        assert route_meta_tag(r) == "main.profile"
